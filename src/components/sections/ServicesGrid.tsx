      ...
      <div className="mx-auto w-[calc(100%-40px)] max-w-content">
        ...
        <div className="grid grid-cols-1 gap-px border border-line bg-line sm:grid-cols-2 lg:grid-cols-4">
          {items.map((item, idx) => {
            const isWeb = idx === 0;
            const cardClass =
              "group block h-full min-h-[220px] bg-surface p-6 transition-colors md:min-h-[245px] md:p-[30px]" +
              (isWeb ? " hover:bg-[#0f0f0f]" : "");
            ...
            const content = (
              <>
                ...
                <h3 className="mb-3 mt-8 flex items-center gap-1.5 text-lg leading-[1.15] md:mt-[42px] md:text-xl">
                  {item.title}
                  {isWeb && (
                    <ArrowUpRight className="h-4 w-4 text-lime opacity-0 transition-opacity group-hover:opacity-100" />
                  )}
                </h3>
            ...
            return (
              <Reveal key={idx} delay={idx * 0.08} className="h-full">
                {isWeb ? (
                  <Link href="/websites" className={cardClass}>
                    {content}
                  </Link>
                ) : (
